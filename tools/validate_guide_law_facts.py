...
def main() -> int:
    ap = argparse.ArgumentParser(description="Validate guide articles against law fact forbidden phrases")
    ...
    facts = load_law_facts(root)
    if not facts:
        print("SKIP: guide_law_facts.json not found")
        return 0
    ...
    for row in rows:
        ...
        if args.skip_greenfield_pending and "greenfield執筆待ち" in norm(row.get("revision_note")):
            continue
        checked += 1
        for hit in scan_row_law_forbidden(row, facts):
            errors.append(f"{slug}: {hit}")

    if errors:
        print(f"FAIL: {len(errors)} law fact hit(s) in {checked} row(s)", file=sys.stderr)
    ...
    print(f"OK: {checked} guide row(s) — no forbidden law-fact phrases")
    return 0
