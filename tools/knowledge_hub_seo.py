...
def hub_prose_html(paragraphs: list[str]) -> str:
    ...
    return "".join(f"<p>{render_inline_markup(p)}</p>" for p in cleaned)
...
def hub_compare_memory_body_html(entry: dict) -> str:
    ...
    if memory_tip and len(memory_tip) >= 48 and not hub_field_is_stub(memory_tip):
        return f'<div class="term-memory-guide"><p>{render_inline_markup(memory_tip)}</p></div>'
    ...
    text = " ".join(parts)
    return f'<div class="term-memory-guide"><p>{render_inline_markup(text)}</p></div>' if text else ""
...
def _legal_explain_wrap(law: str, main_core: str, extras: list[str]) -> str:
    ...
    if core.endswith("について"):
        ...
        text = f"{law}は、{core}。"
    elif re.search(r"(?:です|ます|である|いいます|いう)$", core):
        text = f"{law}は、{core}。"
    else:
...
def glossary_legal_body_html(entry: dict) -> str:
    ...
    for law in split_semicolon(legal):
        ...
        blocks.append(f"<p>{render_inline_markup(explain)}</p>")
    return "\n".join(blocks)
...
def _trap_list_html(pairs: list[tuple[str, str]]) -> str:
    ...
    for wrong, correct in pairs:
        ...
        lis.append(
            f"<li>{_term_item_label('誤り')}：{render_inline_markup(wrong)}"
            f"<br>{_term_item_label('正解の考え方')}：{render_inline_markup(correct)}。</li>"
        )
...
def _checklist_html(items: list[str]) -> str:
    ...
    if not cleaned:
        return ""
    return "<ul>" + "".join(f"<li>{render_inline_markup(item)}</li>" for item in cleaned) + "</ul>"
...
def glossary_exam_choices_body_html(entry: dict) -> str:
    ...
    if intro:
        ...
        parts.append(f"<p>{render_inline_markup(intro)}</p>")
    if checks:
...
def glossary_memory_body_html(entry: dict) -> str:
    ...
    if not memory_tip:
        if short_def and term:
            ...
            return f'<div class="term-memory-guide"><p>{render_inline_markup(text)}</p></div>'
        return ""
    ...
    if labeled:
        ...
        for label, content in labeled:
            ...
            lis.append(
                f"<li>{_term_item_label(label)}：{render_inline_markup(content)}。</li>"
            )
