...
上記は **候補プール**（約120 slug＋アフィリエイト10本）。**本番 CSV の通常ガイドは50本以内** に収め、資格に不要なジャンル・分野別の細分化は省略してください。

**本番で何を残すか** は [guide-portfolio-rewrite-workflow.md](./guide-portfolio-rewrite-workflow.md) の `guide_seo_candidates.csv` で確定（カタログから全行採用しない）。`tools/guide_retire_catalog.py` の **KEEP_CORE** がテンプレ型サイトの骨格。

- 分野が多い資格 → 各 `field-*` は **基礎＋過去問の2本** など最小セットに絞る
- 同ジャンル内の「対象者別」（初学者 / リスキリング / 短期合格）は **1本に統合** するか、最も検索される1本だけ残す
- アフィリエイトは **~10本目安**。50本 cap の**外**に置いてよい（ポートフォリオ整理で削らない）
- 年度・制度更新ごとの差し替え記事（`注意点・更新` ジャンル）
- アフィリエイト記事は **tags に `アフィリエイト`** を付けた行を **10本前後**（ASP リンク済みのみ公開）
