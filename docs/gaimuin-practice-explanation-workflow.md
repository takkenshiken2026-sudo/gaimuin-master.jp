...
| `explanation_choices` | **他の選択肢**（必須） | 下記「他の選択肢の書き方」参照 |
...
**全問必須**（〇×・五肢とも）。未記入だと `tools/q_explanation.py` が汎用文を推論表示する。
...
| 正答 | 書くべき誤答側 | 内容 |
|------|----------------|------|
| `2`（誤っている） | `1:` のみ | 問題文が誤りなので「正しい」を選べない理由 |
| `1`（正しい） | `2:` のみ | 問題文が正しいので「誤っている」を選べない理由 |
...
- [ ] 他問と文体が被っていない
- [ ] `explanation_choices` が全誤肢をカバーしている
- [ ] 各 choice 理由が48字以上・選択肢内容に触れている
- [ ] 禁止定型句がない
